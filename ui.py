...
from chat import chat_with_gpt3_5_turbo
...
class App:
    def __init__(self):
        ...
        # Repeat field
        repeat_label = Label(self.window, text="Repeat:")
        repeat_label.pack(side=LEFT)
        self.repeat_var = IntVar()
        repeat_entry = Entry(self.window, textvariable=self.repeat_var)
        ...
        self.add_button = Button(self.window, text="+", command=self.add_text_field)
    ...
    def insert_text_field(self, index):
        ...
        # add_button = Button(field_frame, text="+", command=lambda idx=index+1: self.insert_text_field(idx))
    ...
    def ok_clicked(self):
        ...
        # print(f"Role: {role_var.get()}, Message: {message_var.get('1.0', 'end').strip()}")

        messages = [{"role": role_var.get(), "content": message_var.get('1.0', 'end').strip()}
                    for role_var, message_var, _ in self.text_fields
                    if role_var.get().strip() and message_var.get('1.0', 'end').strip()]
        ...
        results = []
        results.append({"id": 0, "content": messages[-1]['content']})
        for i in range(1, int(self.repeat_var.get()) + 1):
            result = chat_with_gpt3_5_turbo(messages=messages)
            results.append({"id": i, "content": result})
            print("result: " + result)
        print("STATUS: Start write data")
        write_data(results)
